...
object ProxyServer {
    ...
    val isDebugged: InternalLogLevel = InternalLogLevel.INFO
    ...
    private val urlReg: Pattern = Pattern.compile("^([a-zA-Z\\d.])+\\:*\\d*\\/*", Pattern.CASE_INSENSITIVE)
    val connectProxyResponse: String = "HTTP/1.1 200 Connection established\r\n\r\n"

    def apply() = new Proxy(9050)
    ...
    class Proxy(port: Int = 9050) {
        ...
        def proxyServerPipeline = (pipeline: ChannelPipeline) => {
            ...
            if (isDebugged == InternalLogLevel.DEBUG) {pipeline.addLast("logger", new LoggingHandler(InternalLogLevel.DEBUG))}

            pipeline.addLast("decoder", new HttpRequestDecoder());
            //      pipeline.addLast("aggregator", new HttpChunkAggregator(65536));
            pipeline.addLast("encoder", new HttpResponseEncoder());
            //      pipeline.addLast("chunkedWriter", new ChunkedWriteHandler());
            pipeline.addLast("proxyHandler", new ProxyHandler(chainProxies));
        }
    ...
    class ProxyHandler(chainProxies: mutable.MutableList[InetSocketAddress]) extends SimpleChannelUpstreamHandler {
        ...
        override def messageReceived(ctx: ChannelHandlerContext, me: MessageEvent) {
            ...
            logger.debug("Receive request: {} {}", msg.getMethod + " " + msg.getUri, ctx.getChannel())
            ...
            def processConnectionRequest(request: HttpRequest) {
                def initializeConnectProcess {
                    ...
                    def connectSuccessProcess(future: ChannelFuture) {
                        ...
                        wf.addListener {
                            future: ChannelFuture => logger.info("Finished write: " + future + " to: " + request.getMethod + " " + request.getUri)
                        }

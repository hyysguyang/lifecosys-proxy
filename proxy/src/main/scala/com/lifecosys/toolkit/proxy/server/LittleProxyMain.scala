...
import com.lifecosys.toolkit.proxy.{ ConnectHost, ProxyConfig, ProgrammaticCertificationProxyConfig, Utils }
...
object LittleProxyMain {
  ...
  def main(args: Array[String]) {
    ...
    val server = new Server(8081)(new ProgrammaticCertificationProxyConfig(Some(chainedProxyConfig)))
    val proxy = new Server(8080)(new ProgrammaticCertificationProxyConfig(Some(localConfig)))
  ...
  class Server(port: Int)(implicit proxyConfig: ProxyConfig) extends org.littleshoot.proxy.DefaultHttpProxyServer(port) {
    ...
    val cpm = if (proxyConfig.chainProxies.isEmpty) null else new LittleChainProxyManager() {
      def getChainProxy(request: HttpRequest): String = {
        val connectHost: ConnectHost = proxyConfig.getChainProxyManager.getConnectHost(request.getUri).get
        connectHost.host.toString
      }
      ...
      serverBootstrap.setPipelineFactory(new ChannelPipelineFactory {
        def getPipeline: ChannelPipeline = {
          ...
          val httpRequestHandler = new HttpRequestHandler(ProxyUtils.loadCacheManager, authenticationManager, allChannels, cpm, relayPipelineFactoryFactory, clientChannelFactory) {
            ...
            override def preConnect(clientBootstrap: ClientBootstrap, httpRequest: HttpRequest) {
